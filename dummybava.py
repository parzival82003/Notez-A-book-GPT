...
def display_quiz(questions):
    ...
    for idx, question in enumerate(questions):
        ...
        answer = st.text_input(f"Your Answer for Question {idx+1}:")
        user_answers.append(answer)
    return user_answers

# Quiz scoring function (simple comparison)
    ...
    if st.button("Generate Flashcards"):
        with st.spinner("Generating flashcards..."):
            ...
            if st.button("Take Quiz"):
                user_answers = display_quiz(refined_questions)

                if st.button("Submit Quiz"):
                    score = score_quiz(user_answers, flashcard_answers)
                    st.write(f"Your score is {score} out of {len(refined_questions)}")

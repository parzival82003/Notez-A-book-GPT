...
with tabs[1]:
    ...
    user_query = st.text_input("Ask a Question from the PDFs", key="user_query")
    ...
    if user_query and api_key:
        try:
            ...
            # Get the model - Replace with the correct model ID
            model_id = "models/gemini-pro"  # Replace with your actual model ID

            # Send the user query to the model and get the response using generate_text()
            response = genai.generate_text(model=model_id, prompt=user_query)

            # Display the generated response
            st.write(f"Reply: {response['candidates'][0]['output']}")
